...
本文提出在 Secrel Gateway 中加入 MCP Proxy 能力的产品方案。它是一个 Draft proposal，用来澄清用户价值、产品边界、安全要求和待确认问题；在状态变为 `Accepted` 或合并进主规格之前，不能单独作为实现依据。
...
- Gateway Core：Secrel 共享的请求代理、鉴权、路由、日志和凭证处理核心。
- Credential Store：保存敏感凭证的存储层，本地模式优先使用 OS Keychain，服务端模式使用服务端加密存储。
- Client Key：客户端访问 LLM API 或 MCP Proxy Endpoint 时使用的 API Key。
- Admin 鉴权：管理员访问配置管理接口时使用的 session 或 token。
- Local App：运行在用户本机的 Tauri 桌面 App 和本地 Gateway daemon。
- Server Web Admin：服务端部署中的 Web 管理界面。
...
在 Secrel 的 LLM Gateway 产品模型中，MCP Proxy 是资源代理能力：它与 LLM API Proxy 共用 Gateway Core、Credential Store、Admin 鉴权、Client Key、日志脱敏和本地 / 服务端两种交付形态。这样做的业务价值是让用户在一个可信入口里同时管理 LLM 调用和 Agent 所需的外部工具资源。
...
- 在 Local App 和 Server Web Admin 中提供一致的 MCP 管理体验；服务端 stdio MCP 的默认启用策略仍需确认。
- 与 LLM API Gateway 共享鉴权、安全、日志和可观测性模型。
- Local App 支持 stdio、HTTP 和 SSE 类 MCP 接入；Server Web Admin 支持 HTTP / SSE 类 MCP 接入，服务端 stdio 是否默认启用仍待确认。

## 5. 非目标

- MCP Proxy 不作为独立于 Secrel LLM Gateway 的单独产品线。
- 本 proposal 不定义完整 MCP runtime 调度平台。
- 基础版本不覆盖任意 MCP Server 的高级故障恢复、自动扩缩容或复杂热迁移。
- 基础版本不把所有 MCP 协议差异抽象成统一工具语义。
- Client API Key 和 Admin Token 保持独立权限边界。

## 6. 产品规格

### 6.1 用户流程

用户在 Local App 中新增 MCP Server：

1. 选择 MCP 类型：`stdio`、`http` 或 `sse`。后续新增传输类型时，需要补充对应配置字段和代理行为。
...
用户在 Server Web Admin 中新增 MCP Server：

1. 选择 MCP 类型：`http` 或 `sse`。
2. 填写名称、描述、URL、Headers、所需凭证引用和访问策略。
3. 保存后，Secrel 对敏感字段加密存储，UI 不回显完整 secret。
4. 用户可执行连接测试，查看工具列表和基础 server metadata。
5. 如果服务端后续允许 `stdio` MCP，必须先明确执行环境、命令白名单、权限限制和安全提示。
...
MCP Tool Index 由连接发现生成，用户只管理 MCP Server 配置和启用状态。
...
- 对本地 stdio MCP，Secrel 负责启动和管理真实 MCP Server 进程。
...
本文在 Draft 阶段只描述候选产品边界，不作为实现依据。主产品规格中如提到 MCP Proxy，应仅标记为规划扩展；待本文状态变为 `Accepted` 或 `Merged` 后，再把稳定内容并入主规格。
...
- Proposal 明确 MCP Proxy 是 Secrel Gateway 的资源代理能力。
